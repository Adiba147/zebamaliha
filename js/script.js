...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    function colorActiveMenu() {
        ...
        for (var i = 0; i < section.length; i++) {
            if (isElementInViewport(section[i]) == true) {
                var links_sections = document.getElementById("list-nav").getElementsByTagName("li");
                for (var j = 0; j < links_sections.length; j++) {
                    //console.log("id: " + section[i].id);
                    //console.log("href: " + links_sections[j].firstElementChild.href.split("#")[1]);
                    if (links_sections[j].firstElementChild.href.split("#")[1] == section[i].id) {
                        links_sections[j].firstElementChild.classList.add(versionLi);
                        links_sections[j].firstElementChild.classList.add("main-color");
                    } else {
                        links_sections[j].firstElementChild.classList.remove(versionLi);
                        links_sections[j].firstElementChild.classList.remove("main-color");
                    }
                }
            }
        }
    ...
    function isElementInViewport(el) {
        ...
        const vertInView = (rect.top + rect.top / 1.8 <= windowHeight) && ((rect.top + rect.height) + rect.top / 1.8 >= 0);
        const horInView = (rect.left + rect.top / 1.8 <= windowWidth) && ((rect.left + rect.width) + rect.top / 1.8 >= 0);
